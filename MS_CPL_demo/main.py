...
from cell import APnetWithLossCell, RnetWithLossCell, ReconnetWithLossCell, TrainOneStepCell, AllNetWithLossCell
from mindspore.common.initializer import initializer, XavierUniform, Zero, Normal
...
from models import _AttributeNet, _RelationNet, _param, Recon, All_Net
import sys
...
all_feature_set = sio.loadmat('./feature_test_gan_cub.mat')
...
def train():
    ...
    Allnet = All_Net(APnet, Rnet, Reconnet)
    weights_init_ms(Allnet)
    ...
    Allnet_with_loss = AllNetWithLossCell(Allnet, bce, mse, all_class.shape[0], dataset.train_class.asnumpy().shape[0])
    optimizer = nn.Adam(Allnet.trainable_params(), learning_rate=opt.lr, beta1=0.5, beta2=0.9)
    net_train = TrainOneStepCell(Allnet_with_loss, optimizer)
    net_train.set_train()   
    ...
    for it in range(start_step, 50000 + 1):
        ...
        batch_labels = Tensor(batch_labels)     # [64]
        ...
        batch_images = Tensor(batch_feats)      # [64, 2048]
        

        unsqu = ExpandDims()
        batch_ext = unsqu(batch_images, 0)      # [1 * 64 * 2048]
        batch_ext = mnp.tile(batch_ext, (class_num, 1, 1))
        input_perm = (1, 0, 2)
        batch_ext = transpose(batch_ext, input_perm)    # [64 * 200 * 2048]
        
        
        #unseen samples
        label_batch_val, visual_batch_val = next(iter_)
        att_batch_val   = dataset.attribute[label_batch_val]    # [64, 312]
        ...
        loss = net_train(batch_images, batch_atts, visual_batch_val, att_batch_val, one_hot_labels, sim_labels, support_attr, batch_ext)
