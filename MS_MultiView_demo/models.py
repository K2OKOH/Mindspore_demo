...
from mindspore.common.initializer import Normal, TruncatedNormal, Uniform
from resnet import resnet50_backbone, resnet50_classifier, kaiming_uniform
...
class AlexNet(nn.Cell):
    def __init__(self, num_classes=7):
        ...
        self.features = nn.SequentialCell(
            nn.Conv2d(3, 64, kernel_size=11, stride=4, padding=2, pad_mode='pad', weight_init=TruncatedNormal()),
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=3, stride=2),
            nn.Conv2d(64, 192, kernel_size=5, padding=2, pad_mode='pad', weight_init=Normal()),
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=3, stride=2),
            nn.Conv2d(192, 384, kernel_size=3, padding=1, pad_mode='pad', weight_init=Normal()),
            nn.ReLU(),
            nn.Conv2d(384, 256, kernel_size=3, padding=1, pad_mode='pad', weight_init=Normal()),
            nn.ReLU(),
            nn.Conv2d(256, 256, kernel_size=3, padding=1, pad_mode='pad', weight_init=Normal()),
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=3, stride=2),
        )
        self.classifier = nn.SequentialCell(
            nn.Dropout(),
            nn.Dense(256 * 6 * 6, 4096, weight_init=Uniform()),
            nn.ReLU(),
            nn.Dropout(),
            nn.Dense(4096, 4096, weight_init=Uniform()),
            nn.ReLU(),
            nn.Dense(4096, num_classes, weight_init=Uniform()),
...
class Resnet_DANN(nn.Cell):
    def __init__(self, num_classes=7):
        super(Resnet_DANN, self).__init__()
        self.backbone = resnet50_backbone(class_num=num_classes)
        self.classifier = resnet50_classifier(class_num=num_classes)

        weight_shape = (2, 2048)
        weight = Tensor(kaiming_uniform(weight_shape, a=math.sqrt(5)))
        self.doamin_classifier = nn.Dense(2048, 2, has_bias=True, weight_init=weight, bias_init=0)
        self.grl = GradReverse(1)

    def construct(self, x1):
        feat_s1 = self.backbone(x1)
        # feat_s1 = feat_s1.view(feat_s1.shape[0], -1)
        # label classifier
        class_out_s1 = self.classifier(feat_s1)
        # domain classifier
...
class Resnet_MV3(nn.Cell):
    def __init__(self, num_classes=7):
        super(Resnet_MV3, self).__init__()
        self.backbone = resnet50_backbone(class_num=num_classes)
        self.classifier = resnet50_classifier(class_num=num_classes)

        self.encoder_1 = nn.SequentialCell(
            nn.Dense(2048, 1024, weight_init=Normal()),
            nn.ReLU(),
            nn.Dense(1024, 512, weight_init=Normal()),
        )
        self.encoder_2 = nn.SequentialCell(
            nn.Dense(2048, 1024, weight_init=Normal()),
            nn.ReLU(),
        ...
        self.decoder_1 = nn.SequentialCell(
            nn.Dense(512, 1024, weight_init=Normal()),
            nn.ReLU(),
            nn.Dense(1024, 2048, weight_init=Normal()),
        )
        self.decoder_2 = nn.SequentialCell(
            nn.Dense(512, 1024, weight_init=Normal()),
            nn.ReLU(),
        ...
        self.doamin_1 = nn.SequentialCell(
            nn.Dropout(),
            nn.Dense(512, 256, weight_init=Normal()),
            nn.ReLU(),
            nn.Dropout(),
            nn.Dense(256, 2, weight_init=Normal()),
        )
        self.doamin_2 = nn.SequentialCell(
            nn.Dropout(),
            nn.Dense(512, 256, weight_init=Normal()),
