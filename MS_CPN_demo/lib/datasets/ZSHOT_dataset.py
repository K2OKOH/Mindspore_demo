...
class ZSHOT_DATA(object):

  #def __init__(self, infos, mode):
  def __init__(self, allclasses, trainval_feature, test_seen_feature, test_unseen_feature, trainval_label, test_seen_label, test_unseen_label, mode):
    ...
    if mode == 'train':
      ...
      features, labels = trainval_feature   , trainval_label
    elif mode == 'test-seen':
      features, labels = test_seen_feature , test_seen_label
    elif mode == 'test-unseen':
      features, labels = test_unseen_feature, test_unseen_label
    elif mode == 'test':
      #features = torch.cat([infos['test_seen_feature'], infos['test_unseen_feature']], dim=0)
      concat = ops.Concat()
      features = concat((test_seen_feature, test_unseen_feature))
      labels   = test_seen_label + test_unseen_label
...
class ZSHOT_DATA_new(object):

  #def __init__(self, infos, mode):
  def __init__(self, allclasses, trainval_feature, test_seen_feature, test_unseen_feature, trainval_label, test_seen_label, test_unseen_label, mode):
    super(ZSHOT_DATA_new, self).__init__()
    self.mode               = mode
    #self.allclasses         = copy.deepcopy( infos['allclasses'] )
    self.allclasses         = allclasses
    if mode == 'train':
  ...
  def __getitem__(self, index):
    #assert 0 <= index < len(self), 'invalid index = {:}'.format(index)
    ori_label = self.labels[index]
    #ori_label = Tensor(ori_label.astype(np.int32))
    ori_label = ori_label.astype(np.int32)
    index = Tensor(index.astype(np.int32))
    if self.return_label_mode == 'original':
      return_label = ori_label
    elif self.return_label_mode == 'new':
      #return_label = self.oriCLS2newCLS[ ori_label ]
    ...
    return self.features[index], return_label
...
class AwA2_IMG_Rotate_Save(object):
