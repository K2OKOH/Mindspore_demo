...
import numpy as np
from mindspore.common.initializer import initializer, XavierUniform, Zero, HeUniform
...
def weights_init_ms(net):
    for m in net.cells_and_names():
        if m=='Linear':
            #init.xavier_normal
            HeUniform(m.weight.data)
            #Zero(m.bias)
        
class encoder_template(nn.Cell):    #编码器
    ...
    def __init__(self,input_dim,latent_size,hidden_size_rule, hidden_size_rule_de, output_dim):
        ...
        #self.feature_encoder = nn.Sequential(nn.Linear(input_dim,self.layer_sizes[1]),nn.ReLU(),
                                              #nn.Linear(self.layer_sizes[1],latent_size))  #2048, 1024, 256
        self.feature_encoder = nn.SequentialCell([nn.Dense(input_dim,self.layer_sizes[1]),nn.ReLU(),
                                              nn.Dense(self.layer_sizes[1],latent_size)])  #2048, 1024, 256
        
        #self.apply(weights_init)
        weights_init_ms(self.feature_encoder)
        #self.to(device)
...
class decoder_template(nn.Cell):  #解码器
    ...
    def __init__(self,input_dim,output_dim,hidden_size_rule):  #256,  256, 102
        ...
        self.feature_decoder = nn.SequentialCell([nn.Dense(input_dim,self.layer_sizes[1]),nn.ReLU(),
                                             nn.Dense(self.layer_sizes[1],output_dim)])
        weights_init_ms(self.feature_decoder)
        #self.apply(weights_init)
        #self.to(device)
    def forward(self,x):
