...
def main(xargs):
  ...
  matcontent1   = sio.loadmat('/res101.mat')
  matcontent2   = sio.loadmat('/att_splits.mat')
  ...
  train_dataset       = _DATA(allclasses, trainval_feature, test_seen_feature, test_unseen_feature, trainval_label, test_seen_label, test_unseen_label, 'train')
  ...
  train_dataset_new   = _DATA_new(allclasses, trainval_feature, test_seen_feature, test_unseen_feature, trainval_label, test_seen_label, test_unseen_label, 'train')
  ...
  test_seen_dataset   = _DATA(allclasses, trainval_feature, test_seen_feature, test_unseen_feature, trainval_label, test_seen_label, test_unseen_label, 'test-seen')
  test_seen_loader    = GeneratorDataset(test_seen_dataset, ["batch_idx", "image_feats", "targets"], sampler=train_sampler)
  
  test_unseen_dataset = _DATA(allclasses, trainval_feature, test_seen_feature, test_unseen_feature, trainval_label, test_seen_label, test_unseen_label, 'test-unseen')
  ...
  for iepoch in range(start_epoch, xargs.epochs):
    ...
    train_cls_loss, train_acc = train_model(xargs, train_loader_new, train_features,target_VC,test_class_att,test_adj_dis, \
      train_adj_dis, network1,network2,relation_network,network3,optimizer, config_train, logger, train_classes)
    lr_scheduler.step()
