...
#dataset_root   = Path(os.environ["HOME"]) / 'datasets' / 'zshots'
#save_dir       = Path(os.environ["HOME"]) / 'datasets' / 'zshots' / 'info-files'
dataset_root   = '/media/cqu/D/SWL/zero-shot-propagation/datasets/zshots'
save_dir       = '/media/cqu/D/SWL/zero-shot-propagation/datasets/zshots/info-files'
...
def preprocess_data(dataset_name, save_file):
  ...
  split_dir      = osp.join(dataset_root,'xlsa17','data',dataset_name)
  #matcontent     = sio.loadmat(str(split_dir / "res101.mat"))
  matcontent     = sio.loadmat(osp.join(split_dir, 'res101.mat'))
  ...
  matcontent     = sio.loadmat(osp.join(split_dir ,'att_splits.mat'))
...
if __name__ == '__main__':
  ...
  names = ['AWA2']
  for name in names:
    save_file = './SaveModel/x-{}-data.ckpt'.format(name)
